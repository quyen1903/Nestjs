...
## Spring Boot Enforcement Profile

The Spring service applies these controls without weakening any rule above:

- `SecurityFilterChain` allowlists public foundation/contract routes and denies
  all other requests by default.
- OAuth2 Resource Server/JOSE performs JWT signature and claim verification;
  legacy per-session RS256 verification is isolated in the auth adapter and
  must not enable an HS256 fallback.
- Verified authentication maps to an explicit user, shop, admin, or system
  principal before method/resource authorization.
- `OncePerRequestFilter` generates or validates request IDs, returns the header,
  and clears MDC after every request.
- CORS is an allowlist. Wildcard origins with credentials are prohibited.
- `@RestControllerAdvice` returns typed, redacted errors without production
  stack traces or internal exception details.
- Validated `@ConfigurationProperties` and production placeholders fail fast;
  local defaults are synthetic and non-production.
- Hibernate validates shared schemas. Flyway is forward-only, and existing
  databases require an explicit reviewed baseline.
- Actuator exposes health only by default and never sensitive details.
- Request logging excludes bodies, authorization/cookie headers, credentials,
  tokens, webhook payloads, and confidential commerce data.

The intentional security differences from NestJS are tracked in
`docs/migration/DISCREPANCY_REGISTER.md` and require negative regression tests.

### Legacy access-token transition

- Parse unverified `accountId` and `deviceId` only to perform a bounded active
  session/key lookup; do not authorize from unverified claims.
- Accept only RS256 and an RSA public key of at least 2048 bits.
- Require `iat` and `exp`, enforce bounded clock skew, and enforce key expiry
  when the legacy row supplies it.
- Require the verified token role to equal the active database account type.
- Derive permissions from the active server-side security row, not request
  fields or unverified token permissions.
- Reject the stored refresh token when presented on the bearer access path.
- Return one safe invalid-token response for signature, expiry, role, session,
  and key failures; never log the token or PEM.

### Legacy password transition

- Reproduce the Node 25 Argon2id parameters exactly for existing hashes:
  64 MiB memory, three iterations, parallelism four, 64-byte output, and the
  stored UTF-8 salt.
- Compare decoded hash bytes in constant time and reject malformed hash/salt
  material without exposing it.
- Normalize email and scope credential reads by expected user or shop actor.
- Run the same expensive synthetic hash path for a missing account before
  returning the generic credential failure.
- Never enable password login for admin merely because an account row exists.
- Store failed-login state in `account_security`; five failures for a known
  account lock it for 15 minutes. Return the same safe credential response for
  wrong, missing, wrong-role, and locked accounts.
- Clear failed-attempt and lock timestamps only after successful password
  verification. Retain the suspicious-activity marker for investigation.

### Reusable-token compatibility window

- The owner-approved Phase 2 mode is compatibility-first. Spring writes both
  the legacy raw value and an HMAC-SHA-256 digest to additive shadow columns so
  the unchanged NestJS backend remains a rollback target.
- Spring lookup is digest-first. Raw fallback is limited to legacy rows whose
  digest is null; raw values must never be logged, searched by broad query, or
  returned outside the existing issuance response.
- The HMAC pepper is a production secret, at least 32 characters, and must not
  use the local placeholder.
- Refresh rotation and reuse invalidation execute in one transaction. A reused
  token invalidates the actual owning account's active sessions, including the
  concurrent-refresh loser.
- Password reset tokens are random, expiring, single-use, and use the same
  digest-first transition. Reset consumes the token and revokes active sessions
  transactionally.
- Do not drop or null legacy raw columns before Phase 7 cutover verification,
  rollback rehearsal, and explicit approval. Removal requires a new forward
  migration and a verified data-retention decision.

### Phase 3 account and catalog enforcement

- Manual user/shop registration creates all required account, actor-specific,
  and initial session rows in one transaction. Duplicate identity fields return
  a safe conflict without exposing which physical unique index matched.
- Google OAuth is disabled unless explicitly configured. Require provider-
  verified email, isolate its temporary authorization session from stateless
  bearer APIs, invalidate that session on callback, and require an HTTPS
  production callback.
- Do not persist Google provider access or refresh tokens. Link an existing
  account only by verified normalized email, reject cross-actor links, and keep
  the provider ID unique.
- Every shop catalog mutation derives `shopId` from the verified principal and
  includes it in the product predicate. An active shop-business row is required
  before creation.
- Public catalog SQL always includes active, published, and marketable
  predicates. Client query flags cannot weaken those predicates, and public
  projections exclude authentication, security, business-private, inventory-
  location, and unpublished fields.
- Category mutation is ADMIN/SUPER_ADMIN-only. Deletion is soft and prohibited
  while active products reference the category.

### Phase 4 cart, discount, inventory, and reservation enforcement

- Every cart operation derives `userId` from the verified USER principal.
  Client user/shop/version/old-quantity hints never establish ownership.
- Cart item identity, current name, shop, and price come from active SKU/SPU
  records. Stored cart prices are display snapshots, not checkout authority.
- Discount create/list/delete derives `shopId` from the verified SHOP
  principal and rechecks product ownership for product-scoped policies.
- Discount amount calculation is USER-protected even though the NestJS route
  is public. It reads the verified user's persisted cart and current server
  prices, uses `BigDecimal`, and never accepts client totals as authority.
- Discount expiry, active state, shop/product scope, global limit, and per-user
  limit are checked on quote and again under row lock on consume. Usage count
  and user history update atomically.
- Inventory mutations require verified shop ownership of the SKU. Reservation
  creation locks in deterministic product order and uses an atomic
  non-negative stock predicate.
- Reservation state transitions are idempotent. Release restores stock once,
  consume never restores it, and expired consume/reaper paths restore it once.
  Reservation APIs remain internal until composed by Phase 5 checkout.

### Phase 5 checkout, order, and payment enforcement

- Checkout review and order creation derive the user from the verified USER
  principal. The submitted selection must exactly match the complete active
  cart; client product names, prices, shops, versions, totals, and identities
  do not establish authority.
- Order creation revalidates active/marketable products, current SKU prices,
  inventory, discount policies, and shipping address. Discount consumption,
  per-shop order/items, order-linked reservations, cart clear, and checkout
  idempotency completion commit in one PostgreSQL transaction.
- Checkout uses `BigDecimal`; the legacy integer SKU currency unit is preserved
  and converted to two-decimal Stripe minor units with exact conversion.
- Concurrent checkout retries serialize on the verified user/idempotency scope.
  Reusing a key with a different fingerprint is rejected. A compatibility
  fallback derives a generation key from the cart when the legacy client omits
  the optional header.
- Stripe is disabled unless explicitly configured. Provider secrets remain in
  external configuration, raw card data never reaches this service, and
  client amount/currency/customer/metadata fields are not payment authority.
- The webhook route is public only for provider delivery. It receives exact
  bytes, verifies `Stripe-Signature` before deserialization, and persists the
  event outcome before acknowledging it.
- Payment success consumes the exact order-linked reservations once. Failure
  or cancellation releases them once. A paid event that cannot safely fulfill
  the order creates durable compensation state and an idempotent full refund.
- Refunds require a verified SHOP owner or explicit ADMIN/SUPER_ADMIN, a client
  idempotency key, a confirmed order, and an amount within the remaining
  locally reserved refundable balance. Retry and cumulative-limit records are
  durable before the provider call.
- COD and documented-only payment/order routes are not live behavior and are
  not exposed under compatibility-first migration.
